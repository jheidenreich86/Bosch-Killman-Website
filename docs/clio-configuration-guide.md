...
- [x] Create one intake pipeline with stages: ✔ DONE (2026-07-16) — built as Matter Statuses (Settings → Matters → Matter Statuses): Intake category: `New Lead → Conflict Check → Consultation Scheduled → Consultation Held → Engagement Sent`; Hired category: `Hired`; Did Not Hire category: `Declined`.
- [x] Notifications: ✔ PARTIAL — lead-inbox email notifications are a **per-user** setting (Settings → Personal → Notification settings). Julian's is on. Each attorney must enable their own — added to the same follow-up list as calendar sync.
...
## Demo build (added 2026-07-19)
Static multi-page site generated from the approved v3 design for the board demo:
- Homepage router now links each practice area to its live Clio Grow public intake form (`site/assets/forms.js` holds the link map — one line per area to update).
